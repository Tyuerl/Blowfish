...
namespace Crypt
{
    class Program
    {
        static void Main()
        {
            ...
            string key = "Ключ ";
            uint test = 0x243f6a88;
            BlowFish a = new BlowFish(key);
            string crypted = a.Encryption(text);
            Console.WriteLine("Зашифрованный текст: " + crypted);
            
            BlowFish b = new BlowFish(key);
            string encr = b.Decipher(crypted);
            Console.WriteLine("Расшифрованный текcт: " + encr);
        }
    }
    ...
    class BlowFish
    {
        ...
        // Массив подстановк для черного ящика в сети Фейстеля  F
        ...
        public BlowFish(String key1)
        {
            ...
            keyInBytes = Encoding.Unicode.GetBytes(key1);
            ...
            for (int i = 0; i < N16bit + 2; i++)
            {
                temp = 0x00000000;
                for (int j = 0; j < 4; j++, k++)
                    temp = (temp << 8) | key[k % key_len];
                pi[i] ^= temp;
            }

            for (int i = 0; i < N16bit + 2; i += 2)
            {
                Encryption(ref left, ref right);
                pi[i] = left;
                pi[i + 1] = right;
            }
        ...
        private void Encryption(ref uint left, ref uint right)
        {
            uint xl, xr;
            xl = left;
            xr = right;
        ...
        public String Decipher(String source)
        {
            ...
            return (text.Remove(text.Length - end ));
        }
    }
}